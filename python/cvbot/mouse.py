...
def random_move(reg):
    """
    Region -> None
    Move the mouse to a random location on given
    region 'reg'
    """
    x, y, w, h = reg
    p = randrange(x, x+w), randrange(y, y+h)
    move(p)

def scroll(times=1, dnrt=True, vert=True):
    """
    int, bool, bool -> None
    Use mouse wheel to scroll down or right
    if 'dnrt' is True else scroll up or left
    a number of 'times'
    in the vertical direction if
    'vert' is True else scroll
    in the horizontal direction
    """
    if vert:
        ms.scroll(0, -times if dnrt else times)
    else:
        ms.scroll(-times if dnrt else times, 0)
